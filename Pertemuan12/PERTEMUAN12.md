...
- /usr/bin/su: Sama seperti sudo, perintah su (substitute user) memungkinkan pengguna untuk beralih identitas. Untuk melakukan ini dan memverifikasi password terhadap /etc/shadow, program ini membutuhkan hak root melalui bit SUID.  
- /usr/bin/passwd: File ini membutuhkan SUID karena perintah passwd harus memodifikasi file /etc/shadow (yang hanya bisa ditulis oleh root) untuk menyimpan hash password baru. Dengan SUID, saat pengguna biasa menjalankan passwd, program tersebut berjalan dengan hak akses pemilik file (yaitu root), bukan hak akses pengguna biasa.  
- /usr/bin/sudo: Perintah ini memungkinkan pengguna biasa menjalankan perintah dengan hak root (atau pengguna lain). Oleh karena itu, executable sudo itu sendiri harus memiliki hak root saat dieksekusi agar dapat mengubah konteks pengguna.  
2. Mencari direktori world-writable dan menentukan mana yang valid dan berisiko:  
![Step 2](img/2.png "Step2")  
- Valid: Seluruh direktori yang muncul pada output terminal saya adalah direktori yang valid dan memang dirancang (secara bawaan oleh sistem operasi Linux) untuk dapat ditulis oleh siapa saja (world-writable).  
- Beresiko: Tidak ada direktori yang berisiko. Server lab saya berada dalam kondisi konfigurasi dasar yang bersih.  
